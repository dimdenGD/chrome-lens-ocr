...
Internal method to send a request to the API. You can use it to send a custom request, but you'll have to handle the formdata and dimensions yourself. Original dimensions optional and are used to calculate pixel coordinates of the text. You should supply dimensions before any resizing (hence 'original') if you want to get correct coordinates for original image.
...
```javascript
{
  ...
  // pixelCoords is only included if originalDimensions was supplied to the scan method (should always be the case with Lens class methods, but not with LensCore's scanByData method, unless you supply them yourself)
  pixelCoords: {
